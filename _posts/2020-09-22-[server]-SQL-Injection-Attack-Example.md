...
결론은 워드프레스의 커스텀 플러그인에 SQL injection 공격을 유발하는 안전하지 않은 코드가 존재했다는 것이다. 서비스 환경에 적용시키기 전에 반드시 보안 취약성에 대한 검사를 진행해야한다.

위 예제에 나온 공격자는 매우 허술하고 상당한 양의 흔적들을 남겨 조사하기가 매우 쉬웠지만, 항상 이렇지는 않을 것이다. 훨씬 정교한 공격이 가해 질 수 있음을 명심하자!
